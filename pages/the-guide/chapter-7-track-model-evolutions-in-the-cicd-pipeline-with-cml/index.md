...
{% callout type="note" %} Using GitLab? Go to the [GitLab](#gitlab) section! {%
/callout %}
...
{% callout type="note" %} Finished? Go to the [Switch back to the main
branch](#switch-back-to-the-main-branch) step! {% /callout %}

### GitLab

{% callout type="note" %} Using GitHub? Go to the [GitHub](#github) section! {%
/callout %}
...
**Token name**: _CML_ **Expiration date**: _None_ **Select scopes**: `api`,
...
Select **Create merge request** and change the merge request configuration if
needed. Create the merge request by selecting **Create merge request**.
...
The output of the `git status` command should look like this:
...
- **Previous**: [Chapter 6: Orchestrate the workflow with a CI/CD
  pipeline](/the-guide/chapter-6-orchestrate-the-workflow-with-a-cicd-pipeline)
- **Next**: [Chapter 8: Serve the model with
  MLEM](/the-guide/chapter-8-serve-the-model-with-mlem)
