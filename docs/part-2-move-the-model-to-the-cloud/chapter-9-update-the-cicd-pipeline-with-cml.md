...
commit. However, if the experiment has not been reproduced locally before, the
pipeline will run the experiment from scratch and not save the results to the
DVC remote storage.

In addition, you may want to visualize the results of the experiment in the
...
1. Update the CI/CD pipeline configuration file to save the experiment results
   and visualize them with CML
...
We will enhance the CI/CD pipeline by adding:

- a step to push the `dvc.lock` file to the Git remote
- an automated report comparing new parameters and new metrics to the main
  branch, and published as a comment.
...
=== ":simple-github: GitHub"
    ...
    1. ... restrict trigger to a modification of one of these files (as to avoid
       unnecessary trigger)
    2. ... restrict trigger to a modification of one of these files (as to avoid
       unnecessary trigger).
...
=== ":simple-gitlab: GitLab"
    ...
    **Profile preferences > Access Tokens**.
    ...
    ```yaml title=".gitlab-ci.yml" hl_lines="0-15 19 29-30 54-124"
...
1. Updated the CI/CD pipeline configuration file to add a step to push the
   `dvc.lock` file to the Git remote
2. Updated the CI/CD pipeline configuration file to add an automated report
   comparing new parameters and new metrics to the main branch, and published as a
   comment
3. Pushed the CI/CD pipeline configuration file to Git
